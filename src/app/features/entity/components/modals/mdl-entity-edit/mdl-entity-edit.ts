...
import { EntityCreateDto, EntityDto, EntityUpdateDto } from '@features/entity/models/entity';
...
export class MdlEntityEdit implements OnInit, AfterViewInit, OnDestroy {
  ...
  data = signal<EntityDto | undefined>(undefined);
  ...
  evtOnSubmit(): void{
    ...
    this.confirmationService.confirm({
        header: `¿Actualizar la entidad?`,
        message: 'Confirmar la operación.',
        accept: () => {
            const requestData = this.request;
            this.ldSubmit.set(true);
            
            const subs = this.api.putUpdate(requestData)
            .pipe(finalize(() => {
  ...
  handlerSetValues(data: EntityDto): void{
    this.frm.patchValue({
      type: data.type,
