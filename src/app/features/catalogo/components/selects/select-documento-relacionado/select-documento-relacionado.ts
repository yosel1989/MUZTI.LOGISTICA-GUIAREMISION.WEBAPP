...
@Component({
  selector: 'app-select-documento-relacionado',
  templateUrl: './select-documento-relacionado.html',
  styleUrl: './select-documento-relacionado.scss',
  imports: [
    SelectModule, 
    ReactiveFormsModule, 
    FormsModule
...
export class SelectDocumentoRelacionadoComponent implements OnInit, AfterViewInit, OnDestroy{
    ...
    @Output() selectedChange = new EventEmitter<DocumentoRelacionadoDTO | undefined | null>;

    data = signal<DocumentoRelacionadoDTO[]>([]);
    ldData = signal(false);
    selected = signal<DocumentoRelacionadoDTO | undefined | null>(undefined);
    ...
    loadData(): void{
      ...
      const s = this.catalogoApiService.getDocumentoRelacionados(this.tipo)
      .pipe(finalize(()=>{
